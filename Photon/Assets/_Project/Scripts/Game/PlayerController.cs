...
public class PlayerController : MonoBehaviourPun,IPunObservable
{
    ...
    public float shotPower;// 투사체 발사 힘
    ...
    //Fire를 통해서 생성하는 bomb 객체는 "데드레커닝" (추측항법 알고리즘)을통해서 각 클라이언트들이
    //직접 생성하고, Fire 함수를 호출 받는 시점을 온라인에서 원격으로 호출받음(Remote Procedyre Call)
    [PunRPC]
    private void Fire(Vector3 shotPoint, Vector3 shotDir, PhotonMessageInfo info)
    {
        ...
        //해당 지연시간 만큼 위치, 또는 연산량을 보정해주어야 최대한 원경에서의 플레이가 동기화될 수 있음.
    ...
    {
        //stream을 통해 주고 받는 데이터는 server에서 받는 시간 기준으로 queue형태로 전달된다.
        if (stream.IsWriting)
        {
            //내 데이터를 server로 보냄
            stream.SendNext(hp);
