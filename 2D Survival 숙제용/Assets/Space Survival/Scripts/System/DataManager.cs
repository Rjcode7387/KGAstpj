...
public class DataManager : SingletonManager<DataManager>
{
    ...
    IEnumerator Start()
	{
		...
		OnLoad();
		//씬이 로드될 때마다 호출 될 event
		SceneManager.sceneLoaded += (scene, mode) => {if (scene == SceneManager.GetSceneByName("GameScene"))
			{
	...
	public void OnLoad()
	{
		...
		totalKillCount = PlayerPrefs.GetInt("TotalKillCount", 0); // 저장데이터가 없어서 null 방지를 위해 기본값을 설정할 수 있는 오버로드된 함수가 있다.
