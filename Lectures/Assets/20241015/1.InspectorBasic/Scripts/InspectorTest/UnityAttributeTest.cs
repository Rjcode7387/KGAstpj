...
public class UnityAttributeTest : MonoBehaviour
{
    ...
    //1. SerializeField : 일반적으로 인스펙터에서 가려져야 하는 Private 또는 Protected 변수를
    //Inpector에서 접근 가능하도록 하는 기능
    [SerializeField]
    private int privateIntVar;

    //2. TextArea : Inpector에서 문자열 입력란을 1줄이 아니라 여러 줄로 입력이 가능하도록 표시
    [TextArea(2,5)]
    public string text;
    ...
    //5. Range : int 또는 float 변수의 최대/최소값을 제한하며, 슬라이더바로 값을 바꿀 수 있도록 해줌.
    [Range(0, 1)]
